...
import cn.rongcloud.rtc.base.RCRTCRemoteVideoFrame.RTCBufferI420;
...
public class RTCActivity extends Base implements IRCRTCAudioRouteListener {
    ...
    private SurfaceView remoteFrameLayout;
    ...
    public void rtcClick(View view) {
        if (view.getId() == R.id.btnJoinRoom) {
          ...
          //  remoteFrameLayout.removeAllViews();
            ...
            RCRTCEngine.getInstance().unInit();
           this.finish();
        } else if (view.getId() == R.id.btnAudioRouterInit) {
            RCRTCAudioRouteManager.getInstance().init(RTCActivity.this.getApplicationContext());
            RCRTCAudioRouteManager.getInstance().setOnAudioRouteChangedListener(this);
        } else if (view.getId() == R.id.btnAudioRouterUnInit) {
...
//                remoteFrameLayout.removeAllViews();
//                remoteFrameLayout.addView(videoView);
            }
        });
    }
    ...
    private void create() {
        remoteFrameLayout.getHolder().addCallback(callback);
       // remoteRTCFrameLayout1.getHolder().addCallback(callback1);
